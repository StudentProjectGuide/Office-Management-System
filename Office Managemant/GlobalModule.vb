...
Module GlobalModule
    ...
    Public test_qry
    Public con As New SqlConnection("Server=shahad-pc;Database=dbOffice;Trusted_Connection=false;user id=sa;password=mangalore123;")
    Public Function opendb()
        If conn.State = 1 Then conn.Close()

        conn.Open("Provider=SQLOLEDB.1;Persist Security Info=False;User ID=sa;password=mangalore123;Initial Catalog=dbOffice;Data Source=shahad-pc")
        Return 0
    ...
    Function AmtInWord(ByVal Num As Decimal) As String
        ...
        If InStr(1, strNum, ".") <> 0 Then
            ...
            StrWord = IIf(CDbl(strNum) = 1, "Rupees", "Rupees ") + NumToWord(CDbl(strNum)) + IIf(CDbl(strNumDec) > 0, " and Paise" + cWord3(CDbl(strNumDec)), "")
        Else
            StrWord = IIf(CDbl(strNum) = 1, "Rupees ", "Rupees ") + NumToWord(CDbl(strNum))
        End If
